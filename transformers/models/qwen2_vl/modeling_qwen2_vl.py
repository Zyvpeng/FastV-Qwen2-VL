...
class Qwen2VLAttention(nn.Module):
    ...
    def forward(
        ...
        # print(query_states.shape)
        # print(key_states.shape)
        # print(past_key_value.key_cache)
        if past_key_value is not None:
        ...
        # if len(past_key_value.key_cache)==1:
        #     past_key_value.key_cache[0] = past_key_value.key_cache[0][:,:,1:]
        #     past_key_value.value_cache[0] = past_key_value.value_cache[0][:, :,1:]
...
class Qwen2VLDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        output_attentions: Optional[bool] = True,  #保存attention weight
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                #     image_mask = torch.cat([image_mask[:,0:im_start,:] , image_mask[:,im_start+1:,:]],dim=1)
                #     inputs_embeds = torch.cat([inputs_embeds[:,0:im_start,:] , inputs_embeds[:,im_start+1:,:]],dim=1)
                #
